...
public class POITEST1 {
    ...
    public void testReadExcel() {
        readExcelToObj("f:\\doctemp\\2018版抽样细则~食糖~绵白糖.xlsx");
    }
...
//判断是否具有合并单元格
    ...
    private boolean isMergedRegion(Sheet sheet,int row ,int column) {
        ...
        for (int i = 0; i < sheetMergeCount; i++) {
            CellRangeAddress range = sheet.getMergedRegion(i);
            int firstColumn = range.getFirstColumn();  //0
            int lastColumn = range.getLastColumn();   //0
            int firstRow = range.getFirstRow();             //1
            int lastRow = range.getLastRow();              //6
            if(row >= firstRow && row <= lastRow){
                if(column >= firstColumn && column <= lastColumn){
                    return true;
                }
